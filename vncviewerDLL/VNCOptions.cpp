...
void VNCOptions::LoadFromStruct(VNCOptionsStruct* aStruct)
{
	// process options
	m_listening = aStruct->m_listening;
	m_listenPort = aStruct->m_listenPort;
	m_connectionSpecified = aStruct->m_connectionSpecified;
	m_configSpecified = aStruct->m_configSpecified;
	strcpy_s(m_cmdlnUser, 256, aStruct->m_cmdlnUser); // act: add user option on command line
	strcpy_s(m_clearPassword, 256, aStruct->m_clearPassword); // Modif sf@2002
	m_quickoption = aStruct->m_quickoption; // Modif sf@2002 - v1.1.2
	strcpy_s(m_configFilename, _MAX_PATH, aStruct->m_configFilename); 
	m_restricted = aStruct->m_restricted;

	// default connection options - can be set through Dialog
	m_ViewOnly = aStruct->m_ViewOnly;
	m_NoStatus = aStruct->m_NoStatus;
	m_NoHotKeys = aStruct->m_NoHotKeys;
	m_FullScreen = aStruct->m_FullScreen;
	m_Directx = aStruct->m_Directx;
	m_ShowToolbar = aStruct->m_ShowToolbar;

    autoDetect = aStruct->autoDetect;
	m_Use8Bit  = aStruct->m_Use8Bit;	// sf@2005 - Now has 7 possible values (defines in rfbproto.h file)
						// 0 : Full colors
						// 1 : 256 colors
						// 2 : 64 colors
						// 3 : 8 colors
						...
						// 5 : 4 colors
						// 6 : 2 Grey colors
	m_PreferredEncoding = aStruct->m_PreferredEncoding;

	m_SwapMouse = aStruct->m_SwapMouse;
	m_Emul3Buttons = aStruct->m_Emul3Buttons;
	m_JapKeyboard = aStruct->m_JapKeyboard;
 	m_Emul3Timeout = aStruct->m_Emul3Timeout;
 	m_Emul3Fuzz = aStruct->m_Emul3Fuzz;
	m_Shared = aStruct->m_Shared;
	m_DeiconifyOnBell = aStruct->m_DeiconifyOnBell;
	m_DisableClipboard = aStruct->m_DisableClipboard;
	m_localCursor = aStruct->m_localCursor;
	m_throttleMouse = aStruct->m_throttleMouse; // adzm 2010-10
	m_scaling = aStruct->m_scaling;
	m_fAutoScaling = aStruct->m_fAutoScaling;
	m_scale_num = aStruct->m_scale_num;
	m_scale_den = aStruct->m_scale_den; // Numerator & denominator
	// Tight specific
	m_useCompressLevel = aStruct->m_useCompressLevel;
	m_compressLevel = aStruct->m_compressLevel;
	m_enableJpegCompression = aStruct->m_enableJpegCompression;
	m_jpegQualityLevel = aStruct->m_jpegQualityLevel;
	m_requestShapeUpdates = aStruct->m_requestShapeUpdates;
	m_ignoreShapeUpdates = aStruct->m_ignoreShapeUpdates;

	// Modif sf@2002 - Server Scaling
	m_nServerScale = aStruct->m_nServerScale; // Divider of the Target Server's screensize
	// Reconnect
	m_reconnectcounter = aStruct->m_reconnectcounter;
	// Modif sf@2002 - Cache
	m_fEnableCache = aStruct->m_fEnableCache;
	// Modif sz@2002 - DSM Plugin
	m_fUseDSMPlugin = aStruct->m_fUseDSMPlugin;
	strcpy_s(m_szDSMPluginFilename, _MAX_PATH, aStruct->m_szDSMPluginFilename); 
	m_oldplugin = aStruct->m_oldplugin;

	// sf@2003 - Autoscaling
	m_saved_scale_num = aStruct->m_saved_scale_num;
	m_saved_scale_den = aStruct->m_saved_scale_den;
	m_saved_scaling = aStruct->m_saved_scaling;

	// Keyboard can be specified on command line as 8-digit hex
	strcpy_s(m_kbdname, 9, aStruct->m_kbdname); 
	m_kbdSpecified = aStruct->m_kbdSpecified;

	// Connection options we don't do through the dialog
	// Which encodings do we allow?
	//m_UseEnc = aStruct->m_UseEnc[LASTENCODING+1];

	strcpy_s(m_host_options, 256, aStruct->m_host_options); 
	m_port = aStruct->m_port;

	strcpy_s(m_proxyhost, 256, aStruct->m_proxyhost); 
	m_proxyport = aStruct->m_proxyport;
	m_fUseProxy = aStruct->m_fUseProxy;

	m_selected_screen = aStruct->m_selected_screen;

    // Logging
    m_logLevel = aStruct->m_logLevel;
    m_logToFile = aStruct->m_logToFile; 
	m_logToConsole = aStruct->m_logToConsole;
	strcpy_s(m_logFilename, _MAX_PATH, aStruct->m_logFilename); 

	// for debugging purposes
	m_delay = aStruct->m_delay;

	// sf@2007 - AutoReconnect
	m_autoReconnect = aStruct->m_autoReconnect;

	// Fix by Act : No User and/Or password if the first VNC connection is rejected
	m_NoMoreCommandLineUserPassword = aStruct->m_NoMoreCommandLineUserPassword;

	m_fExitCheck = aStruct->m_fExitCheck; //PGM @ Advantig
    m_FTTimeout = aStruct->m_FTTimeout;
    m_keepAliveInterval = aStruct->m_keepAliveInterval;
	m_socketKeepAliveTimeout = aStruct->m_socketKeepAliveTimeout; // adzm 2010-08

	//adzm - 2009-06-21
	m_fAutoAcceptIncoming = aStruct->m_fAutoAcceptIncoming;

	//adzm 2009-07-19
	m_fAutoAcceptNoDSM = aStruct->m_fAutoAcceptNoDSM;

	//adzm 2010-05-12
	m_fRequireEncryption = aStruct->m_fRequireEncryption;

	//adzm 2010-07-04
	m_preemptiveUpdates = aStruct->m_preemptiveUpdates;
}

void VNCOptions::SaveToStruct(VNCOptionsStruct* aStruct)
{
	// process options
	aStruct->m_listening = m_listening;
	aStruct->m_listenPort = m_listenPort;
	aStruct->m_connectionSpecified = m_connectionSpecified;
	aStruct->m_configSpecified = m_configSpecified;
	strcpy_s(aStruct->m_cmdlnUser, 256, m_cmdlnUser); // act: add user option on command line
	strcpy_s(aStruct->m_clearPassword, 256, m_clearPassword); // Modif sf@2002
	aStruct->m_quickoption = m_quickoption; // Modif sf@2002 - v1.1.2
	strcpy_s(aStruct->m_configFilename, _MAX_PATH, m_configFilename);
	aStruct->m_restricted = m_restricted;

	// default connection options - can be set through Dialog
	aStruct->m_ViewOnly = m_ViewOnly;
	aStruct->m_NoStatus = m_NoStatus;
	aStruct->m_NoHotKeys = m_NoHotKeys;
	aStruct->m_FullScreen = m_FullScreen;
	aStruct->m_Directx = m_Directx;
	aStruct->m_ShowToolbar = m_ShowToolbar;

    aStruct->autoDetect = autoDetect;
	aStruct->m_Use8Bit = m_Use8Bit ;	// sf@2005 - Now has 7 possible values (defines in rfbproto.h file)
						// 0 : Full colors
						// 1 : 256 colors
						// 2 : 64 colors
						// 3 : 8 colors
						...
						// 5 : 4 colors
						// 6 : 2 Grey colors
	aStruct->m_PreferredEncoding = m_PreferredEncoding;

	aStruct->m_SwapMouse = m_SwapMouse;
	aStruct->m_Emul3Buttons = m_Emul3Buttons;
	aStruct->m_JapKeyboard = m_JapKeyboard;
 	aStruct->m_Emul3Timeout = m_Emul3Timeout;
 	aStruct->m_Emul3Fuzz = m_Emul3Fuzz;
	aStruct->m_Shared = m_Shared;
	aStruct->m_DeiconifyOnBell = m_DeiconifyOnBell;
	aStruct->m_DisableClipboard = m_DisableClipboard;
	aStruct->m_localCursor = m_localCursor;
	aStruct->m_throttleMouse = m_throttleMouse; // adzm 2010-10
	aStruct->m_scaling = m_scaling;
	aStruct->m_fAutoScaling = m_fAutoScaling;
	aStruct->m_scale_num = m_scale_num;
	aStruct->m_scale_den = m_scale_den; // Numerator & denominator
	// Tight specific
	aStruct->m_useCompressLevel = m_useCompressLevel;
	aStruct->m_compressLevel = m_compressLevel;
	aStruct->m_enableJpegCompression = m_enableJpegCompression;
	aStruct->m_jpegQualityLevel = m_jpegQualityLevel;
	aStruct->m_requestShapeUpdates = m_requestShapeUpdates;
	aStruct->m_ignoreShapeUpdates = m_ignoreShapeUpdates;

	// Modif sf@2002 - Server Scaling
	aStruct->m_nServerScale = m_nServerScale; // Divider of the Target Server's screensize
	// Reconnect
	aStruct->m_reconnectcounter = m_reconnectcounter;
	// Modif sf@2002 - Cache
	aStruct->m_fEnableCache = m_fEnableCache;
	// Modif sz@2002 - DSM Plugin
	aStruct->m_fUseDSMPlugin = m_fUseDSMPlugin;
	strcpy_s(aStruct->m_szDSMPluginFilename, _MAX_PATH, m_szDSMPluginFilename);
	aStruct->m_oldplugin = m_oldplugin;

	// sf@2003 - Autoscaling
	aStruct->m_saved_scale_num = m_saved_scale_num;
	aStruct->m_saved_scale_den = m_saved_scale_den;
	aStruct->m_saved_scaling = m_saved_scaling;

	// Keyboard can be specified on command line as 8-digit hex
	strcpy_s(aStruct->m_kbdname, 9, m_kbdname);
	aStruct->m_kbdSpecified = m_kbdSpecified;

	// Connection options we don't do through the dialog
	// Which encodings do we allow?
	//m_UseEnc = aStruct->m_UseEnc[LASTENCODING+1];

	strcpy_s(aStruct->m_host_options, 256, m_host_options);
	aStruct->m_port = m_port;

	strcpy_s(aStruct->m_proxyhost, 256, m_proxyhost);
	aStruct->m_proxyport = m_proxyport;
	aStruct->m_fUseProxy = m_fUseProxy;

	aStruct->m_selected_screen = m_selected_screen;

    // Logging
    aStruct->m_logLevel = m_logLevel;
    aStruct->m_logToFile = m_logToFile;
	aStruct->m_logToConsole = m_logToConsole;
	strcpy_s(aStruct->m_logFilename, _MAX_PATH, m_logFilename);

	// for debugging purposes
	aStruct->m_delay = m_delay;

	// sf@2007 - AutoReconnect
	aStruct->m_autoReconnect = m_autoReconnect;

	// Fix by Act : No User and/Or password if the first VNC connection is rejected
	aStruct->m_NoMoreCommandLineUserPassword = m_NoMoreCommandLineUserPassword;

	aStruct->m_fExitCheck = m_fExitCheck; //PGM @ Advantig
    aStruct->m_FTTimeout = m_FTTimeout;
    aStruct->m_keepAliveInterval = m_keepAliveInterval;
	aStruct->m_socketKeepAliveTimeout = m_socketKeepAliveTimeout; // adzm 2010-08

	//adzm - 2009-06-21
	aStruct->m_fAutoAcceptIncoming = m_fAutoAcceptIncoming;

	//adzm 2009-07-19
	aStruct->m_fAutoAcceptNoDSM = m_fAutoAcceptNoDSM;

	//adzm 2010-05-12
	aStruct->m_fRequireEncryption = m_fRequireEncryption;

	//adzm 2010-07-04
	aStruct->m_preemptiveUpdates = m_preemptiveUpdates;
}
