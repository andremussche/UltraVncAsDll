...
#ifdef UNDER_CE
int WINAPI WinMain_(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR szCmdLine, int iCmdShow)
#else
...
VNCviewerApp32 * m_app;

extern "C" DLLEXPORT /*export without name mangling*/
int VncViewerDll_Init(HINSTANCE hInstance)
{
#ifdef IPP
	InitIpp();
#endif
...
//	RegisterLinkLabel(m_hInstResDLL);
	...
	// The state of the application as a whole is contained in the one app object
    m_app = new VNCviewerApp32(hInstance, "");

    console = m_app->m_options.m_logToConsole;

	return 0; //OK
}

ClientConnection* lastconn;

extern "C" DLLEXPORT /*export without name mangling*/
HWND VncViewerDll_NewConnection(char* aHost, int aPort)
{
	if (!m_app) { return 0; };
	...
	//return h; //conn->m_hwndMain;	
	//return 0; //OK
}

extern "C" DLLEXPORT /*export without name mangling*/
HWND VncViewerDll_ToolbarHandle()
{
	if (!m_app) { return 0; };
	return lastconn->m_hwndTB;
}

extern "C" DLLEXPORT /*export without name mangling*/
HWND VncViewerDll_ToolbarWindow()
{
	if (!m_app) { return 0; };
	return lastconn->m_hwndTBwin;
}
...
extern "C" DLLEXPORT /*export without name mangling*/
int VncViewerDll_GetOptions(VNCOptionsStruct* aStruct)
{
	if (!m_app) { return -1; };

	m_app->m_options.SaveToStruct(aStruct);
	return 0; //OK
}

extern "C" DLLEXPORT /*export without name mangling*/
int VncViewerDll_SetOptions(VNCOptionsStruct* aStruct)
{
	if (!m_app) { return -1; };

	m_app->m_options.LoadFromStruct(aStruct);
	return 0; //OK
}
...
{
	VncViewerDll_Init(hInstance);

	m_app->m_options.m_NoStatus = true;  //no pending status popup
	//g_disable_sponsor = true;  //no advertising
	strcpy(m_app->m_options.m_clearPassword, "test" + '\0');  //no "enter password" popup
	m_app->m_options.m_nServerScale = 2;


	HWND h = VncViewerDll_NewConnection("localhost", 5900);
	ShowWindow(h, SW_SHOWNORMAL);
	//VncViewer_NewConnection("localhost", 5900);
	//m_app->NewConnection(false);
